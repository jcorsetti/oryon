...
from timm.models.layers import Mlp, DropPath, to_2tuple, to_ntuple
...
class MLPHead(nn.Module):
    ...
    def __init__(self, hidden_dim=64, pooling_size=(4, 4)) -> None:
        super().__init__()
        ...
        self.visualconv = nn.Linear(in_features=hidden_dim, out_features=hidden_dim)
        self.textconv = nn.Linear(in_features=hidden_dim, out_features=hidden_dim)
        self.act = nn.ReLU()
    ...
    def forward(self, x, guidance):
        """
        Arguments:
            x: B, C, T, H, W    # B, 128, 1, 24, 24
            guidance: B, T, C   # B, 1, 128
        """
        B, _, T, H, W = x.size()
        ...
        *_, H_pool, W_pool = x_pool.size()
        ...
        if guidance is not None:
            guidance = repeat(guidance, 'B T C -> (B H W T) C', H=H_pool, W=W_pool, T=T)

        x_pool = self.act(self.visualconv(x_pool))
        guidance = self.act(self.textconv(guidance))
        x_summed = x_pool + guidance
        x_pool = self.MLP(x_summed) + x_pool

        x_pool = rearrange(x_pool, '(B H W T) C -> (B T) C H W', H=H_pool, W=W_pool, T=T)
...
class AggregatorLayer(nn.Module):
    def __init__(self, hidden_dim=64, text_guidance_dim=512, appearance_guidance=512, nheads=4, input_resolution=(20, 20), pooling_size=(5, 5), window_size=(10, 10), attention_type='linear', guidance_head='default') -> None:
        super().__init__()
        ...
        elif self.guidance_head == 'mlp':
            self.attention = MLPHead(hidden_dim, pooling_size=pooling_size)
        elif self.guidance_head == 'none':
            self.attention_head = None
        else:
            raise RuntimeError(f"Guidance head {self.guidance_head} not implemented.")
...
class ImageTextFusion(nn.Module):
    def __init__(self, 
        ...
        fusion_guidance_input_dim=512,        
        fusion_guidance_output_dim=128,
        num_layers=2, 
        nheads=4, 
        hidden_dim=128,
        pooling_size=(6, 6),
        in_feature_resolution=(24, 24),
        window_size=12,
        ...
        self.layers = nn.ModuleList([
            AggregatorLayer(
                hidden_dim=hidden_dim, text_guidance_dim=text_guidance_proj_dim, appearance_guidance=fusion_guidance_output_dim, 
                nheads=nheads, input_resolution=in_feature_resolution, pooling_size=pooling_size, window_size=window_size, attention_type=attention_type, guidance_head='default'
            ) for _ in range(num_layers)
        ]).to(device)
        
        self.conv1 = nn.Conv2d(num_prompts, hidden_dim, kernel_size=7, stride=1, padding=3).to(device)
        self.clip_conv = torch.nn.Conv1d(in_channels=1024, out_channels=768, kernel_size=1).to(device)
        
        if self.use_fusion_guidance:
            self.guidance_projection = nn.Sequential(
                nn.Conv2d(fusion_guidance_input_dim, fusion_guidance_output_dim, kernel_size=3, stride=1, padding=1),
                nn.ReLU()
            ).to(device)
    ...
    def project_featmap(self, image : torch.Tensor) -> torch.Tensor:
        
        B,D,H,W = image.shape

        image = self.clip_conv(rearrange(image, 'B D H W -> B D (H W)',B=B,D=D,H=H,W=W))
        D = image.shape[1]
        image = rearrange(image, 'B D (H W) -> B D H W',B=B,D=D,H=H,W=W)
    ...
    def correlation(self, img_feats, text_feats):
        ...
        corr = torch.einsum('bchw, btpc -> bpthw', img_feats, text_feats) # B P 1 H W
        return corr
    ...
    def corr_embed(self, x):
        ...
        corr_embed = rearrange(corr_embed, '(B T) C H W -> B C T H W', B=B) # B 128 1 H W
        return corr_embed
        
    def forward(self, img_feats: Tensor, text_feats: Tensor, app_guide: List[Tensor]):
        """
        Arguments:
            img_feats: (B, C, H, W)
            text_feats: (B, T, P, C)
            app_guide: tuple of (B, C, H, W)
        """
        img_feats = self.project_featmap(img_feats)
        corr = self.correlation(img_feats, text_feats)
        corr_embed = self.corr_embed(corr)
        
        proj_guide, projected_text_guidance = None, None
        if self.guidance_projection is not None:
            proj_guide = self.guidance_projection(app_guide[0])
        ...
        for layer in self.layers:
            corr_embed = layer(corr_embed, proj_guide, projected_text_guidance)

        return corr_embed

def get_fusion_module(args: DictConfig, device:str) -> ImageTextFusion:
    return ImageTextFusion(device, use_fusion_guidance=args.image_encoder.use_cost_guidance)